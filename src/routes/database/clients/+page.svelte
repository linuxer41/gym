<script lang="ts">
	...
	import ClientProfile from '$lib/components/profile/ClientProfile.svelte';
...
<div class="grid overflow-hidden grid-rows-[auto_1fr] gap-3 p-3 pt-8 w-full h-full bg-white">
	{#if targetSubscriber}
		<ClientProfile subscriber={targetSubscriber} on:close={()=>targetSubscriber = null} />
	{:else}
		...
		<SubscriberTable
			...
			on:edit
			on:delete
			on:select={(e) => (targetSubscriber = e.detail)}
		/>
	{/if}
</div>
