			...
			<div class="cards-contaier p-2">
				...
				<div class="w-full rounded box-sahdow p-3 grid  content-start">
					<div class="grid gap-3 grid-cols-[1fr_auto]">
						...
						<button
							class="bg-slate-500 place-self end w-min hover:bg-slate-700 text-white font-bold py-2 px-4 rounded"
							on:click={() => {
								createPlan = true;
							}}
						>
				...
				{#each memberships as membership}
					...
					<div class="w-full rounded bg-lime-500 p-3 grid  grid-cols-[1fr_auto]">
						<div class="grid gap-1 content-start">
							...
							<!-- Plan chip tailwind -->
							<div
								class="inline-flex w-min whitespace-nowrap items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800"
							>
								{plans.find((plan) => plan.id === membership.plan_id)?.name}
							</div>
						</div>
						<div class="grid gap-1 place-items-end">
							<button class="text-blue-500" on:click={() => (toEditMembership = membership)}
								>Editar</button
							>
							<DeleteButton
								title="eliminar"
