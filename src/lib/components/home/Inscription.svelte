<script lang="ts">
	...
	function addReferedSubscriber(subscriber: Subscriber) {
	...
	function removeReferedSubscriber(subscriber: Subscriber) {
		const index = referredSubscribers.findIndex((s) => s.id === subscriber.id);
		if (index !== -1) {
								...
								<div class="bg-white shadow rounded-lg overflow-hidden">
											...
											>
												...
												<button
													on:click|preventDefault={() =>
														removeReferedSubscriber(referredSubscriber)}
									...
									{#if subscribers?.length > 0}
										...
										<div class="px-4 py-5 sm:p-6 max-h-80 overflow-auto">
											<SubscriberTable
												data={subscribers}
												on:select={(e) => addReferedSubscriber(e.detail)}
												title=""
											/>
										</div>
									{/if}
